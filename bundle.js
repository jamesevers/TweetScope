...
var Maps = {
  initialState: function initialState() {
    ...
    var hourlyTweets = Util.filterByTime(allData, initialHour);
    var wordCounts = Util.wordCounts(hourlyTweets);
    ...
    d3.json("https://d3js.org/us-10m.v1.json", function (error, us) {
      ...
      statesGroup.selectAll("path").attr("class", "states").data(topojson.feature(us, us.objects.states).features).enter().append("path").attr("d", path).attr("fill", "#3850a0").attr("opacity", .8);

      svg.append("path").attr("class", "border").attr("d", path(topojson.mesh(us, us.objects.states, function (a, b) {
        return a !== b;
      })));
    });
  ...
  drawMap: function drawMap() {
    ...
    d3.json("https://d3js.org/us-10m.v1.json", function (error, us) {
      ...
      svg.append("g").attr("class", "states").selectAll("path").data(topojson.feature(us, us.objects.states).features).enter().append("path").attr("d", path).attr("fill", "blue").attr("opacity", .8);
  ...
  placeCoords: function placeCoords(data) {

    var svg = d3.select("svg");
    var tweets = Util.parseData(data);
    var projection = d3.geoAlbersUsa();
    projection.scale([1300]);

    var div = d3.select("body").append("div").attr("class", "tooltip").style("opacity", 0);

    svg.selectAll("circle").data(tweets).enter().append("circle").attr("cx", function (d) {
      var coord = d.coordinates.coordinates;
      return projection(coord)[0] - 5; // + 60
    }).attr("cy", function (d) {
      var coord = d.coordinates.coordinates;
      return projection(coord)[1] + 55; // + 55
    }).attr("r", "1px").attr("fill", "url(#RadialGradient)").attr("class", "tweet-initial").style("opacity", .8).on("mouseover", function (d) {
      div.transition().duration(200).style("opacity", .9).style("fill", "yellow");
      div.html(d.text).style("left", d3.event.pageX + "px").style("top", d3.event.pageY - 28 + "px").style("background", "white").style("color", "#1D2951");
    }).on("mouseout", function (d) {
      div.transition().duration(500).style("opacity", 0);
    });
    d3.selectAll("circle").transition().duration(500).style("fill", "orange").attr("r", "6px").transition().duration(500).style("fill", "yellow").attr("r", "3px");
  },
...
module.exports = {
  ...
  switchData: function switchData(newValue) {
    Maps.clearCoords();
    var allData = Util.loadAllData();
    var hourlyTweets = Util.filterByTime(allData, newValue);
    var wordCounts = Util.wordCounts(hourlyTweets);
    Util.topWordCounts(wordCounts);
    Util.getDisplay(hourlyTweets);
